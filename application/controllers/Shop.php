...
class Shop extends CI_Controller {
    ...
    public function index() {
        ...
        $data['categories'] = $this->shop->getCategories();
        $data['sub_categories'] = $this->shop->getSub_categories();           

        if(!empty($this->session->userdata('user_login'))){
            $wishlist_count = $this->shop->check_prod_id('', $data['user_id']);
        ...
        $data['prod_count'] = $this->shop->getProdetails(true, '', '', $category_id, '', '');
        ...
        $data['min_price'] = $this->shop->getPrice(false);
        $data['min_price'] = $data['min_price'][0];
        $data['min_price'] = explode('.', $data['min_price']['price']);
        $data['max_price'] = $this->shop->getPrice(true);
        $data['max_price'] = $data['max_price'][0];
        $data['max_price'] = explode('.', $data['max_price']['price']);
        $data['products'] = $this->shop->getProdetails(false, $config['per_page'], $page, $category_id, '', '');
        $data['banner_name'] = $this->shop->getBanners();
//        echo '<pre>';print_r($data['banner_name']);exit;
     ...
     //   $data['user_id'] = $this->session->userdata('id');
        $data['wishlist_products'] = $this->wishlist->getAddedProducts($data['user_id']);
        //$this->pagination->initialize($config);           //need to verify
...
*/
    ...
    public function add_to_cart($product_id, $price, $prod_quantity) {
        ...
        $prod_limit = $this->shop->getProdQuantity($product_id);
        $prod_limit = $prod_limit->product_quantity;

        $product_quantity = $this->shop->getProdById($product_id);
...
//                    echo '<pre>';print_r($existing_cart_data);exit;
                    if ($prod_quantity == 1) {
    ...
    public function add_to_wishlist($product_id) {
        ...
        if (!empty($this->session->user_login)) {
           ...
           // $data['user_id'] = $this->session->userdata('id');
            $result_check_product_id = $this->shop->check_prod_id($data['product_id'], $data['user_id']);
            $wishlist_count = $this->shop->check_prod_id('', $data['user_id']);
            $wishlist_count = COUNT($wishlist_count);
            $this->session->set_userdata('wishlist_count', $wishlist_count);
            if ($result_check_product_id) {
